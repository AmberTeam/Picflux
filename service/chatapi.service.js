...
class ChatApiService {
    ...
    async storeMsg(owner, msg) {
        return new Promise((resolve, reject) => {
            ...
            DBAgent.db.run(`INSERT INTO messages(_id, chatid, owner, payload, timestamp, seen, type, refer) VALUES("${msg._id}", "${msg.chatid}", "${owner.id}", ?, ${Date.now()}, "${msg.seen}", "${msg.type}", "${msg.refer ? msg.refer : "null"}")`, [msg.payload], (err, data) => {
                if(err) {
                    console.error(err)
                    return reject(ApiError.BadRequest(ApiError.econfig.bad_request))
                }
