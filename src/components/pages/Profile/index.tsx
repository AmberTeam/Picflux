                ...
                <div className={cl.ProfileEdit_container}>
                    ...
                    <div className={cl.Data_edit}>
                        <div className={cl.DataEdit_header}>
                            {translate("profile.edit.usn")}
                        </div>
                        <div className={cl.DataEdit_body}>
                            <input
                                className={`${profileEditErrs.includes('username') ? cl.Err : cl.Default}`}
                                placeholder={translate("profile.edit.usn_e")}
                                defaultValue={store.user.username}
                                onChange={(e) => setPEC({...profileEditConfig, username: e.target.value})}
                            />
                            {profileEditErrs.includes('username') && <span className={cl.Err_txt}>{translate("profile.edit.ex")}</span>}
                        </div>
                    </div>
                    <div className={cl.Data_edit}>
                        <div className={cl.DataEdit_header}>
                            {translate("profile.edit.bio")}
                        </div>
                        <div className={cl.DataEdit_body}>
                            <input
                                className={cl.Username_edit}
                                placeholder={translate("profile.edit.bio_ex")}
                        ...
                        <div className={cl.Content_header}>
                            ...
                            <div className={cl.Profile_stats}>
                                <div className={cl.Stats_container}>
                                    <div className={`${cl.Stat_friends} ${cl.Stat}`}>
                                        ...
                                        <span>{translate("profile.intro.stats.fr")}</span>
                                    </div>
                                    <div className={cl.Stat_separator}></div>
                                    <div className={`${cl.Stat_status} ${cl.Stat}`}>
                                        <h3> Kino Geek </h3>
                                        <span>{translate("profile.intro.stats.id")}</span>
                                    </div>
                                    <div className={cl.Stat_separator}></div>
                                    <div className={`${cl.Stat_status} ${cl.Stat}`}>
                            ...
                            <div className={cl.Profile_actions}>
                                    ...
                                    <button className={`${cl.Subscribe_btn} ${""}`} onClick={() => friendshipAction()}>
                                        {
                                            followActionLoading 
                                                ...
                                                :
                                                <>
                                                    <span>{user.subscribed ? translate("profile.intro.actions.uns") : translate("profile.intro.actions.sub")}</span>
                                                    {
                                                        user.subscribed 
                                                            ?
                                ...
                                <button className={cl.Subscribe_btn}>
                                    <span>{translate("profile.intro.actions.sh")}</span>
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16">
                                ...
                                </button>
                                {
                                    ...
                                    <a className={cl.Subscribe_btn} href={`/inbox/redirect/${user.id}`}>
                                        <span>{translate("profile.intro.actions.send")}</span>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16">
                                ...
                                <AllowOwner compareid={user.id}>
                                    <button className={cl.Subscribe_btn} onClick={() => setPEMA(true)}>
                                        <span>{translate("profile.intro.actions.ed")}</span>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16">
